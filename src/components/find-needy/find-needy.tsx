...
export function FindNeedy () {
  ...
  function createMarkers (posts: IPostMinimal[]) {
    setMarkers(
      posts.map((e) => {
        return new mapboxgl.Marker().setLngLat([parseFloat(e.longitude), parseFloat(e.latitude)]);
      })
    );
  }
              ...
              <div className={styles.line1} key={post.id}>
                <div className={styles.name}>
                  <div className={styles.para1}>
                    <p className={styles.cont2text1}>Posted by {post.postedBy.name}</p>
                  </div>
                ...
                <div className={styles.nameImg1}>
                  <p className={styles.userImg1}>
                    <img src='/assets/find-needy/user.png' alt='' height='46px' width='46px' />
                  </p>
                </div>
              </div>
            </Link>
