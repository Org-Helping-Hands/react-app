...
export function DoPost () {
  ...
  function handleFileChange (data: any) {
    const file = data.target.files as FileList;
  ...
  function toggleTagInModal (name: TTag) {
    const tagExist = selectedTagsInModal.includes(name);

    if (tagExist) {
      const updatedData = selectedTagsInModal.filter((e) => e !== name);
  ...
  function handleRemoveTag (name: TTag) {
    const updatedData = selectedTags.filter((e) => e !== name);
  ...
  function handlePostClick () {
    if (coords) {
      const neededItems = selectedTags.map((e) => e.name);
      dopost(coords?.latitude, coords?.longitude, description, images, neededItems).then((_) =>
    ...
    <>
      ...
      <div className={styles.reqpost}>
                ...
                <label className={styles.filebutton}>
                  ...
                  <span>
                    <input
                      ...
                      className={styles.input_file}></input>
        ...
        <div className={styles.tags_selected}>
            ...
            {selectedTags.map((ele, index) => (
              <div key={index} className='col-6 col-sm-4 col-md-3'>
                <button type='button' className={styles.tagbtn}>
                  ...
                  <span className='iconify-wrapper' onClick={() => handleRemoveTag(ele)}>
          ...
          <div className='row'>
            <button className={`btn btn-primary ${styles.postbtn}`} onClick={handlePostClick}>
              Post
            </button>
          </div>
        </div>
